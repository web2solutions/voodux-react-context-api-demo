...
export default function Deposits (props) {
  ...
  useEffect(() => {
    // console.debug('------->>>>> Deposits.js mount events', foundation.stopListenTo)
    onAddDocEventListener = foundation.on(`collection:add:${props.entity.toLowerCase()}`, async function (eventObj) {
    ...
    onEditDocEventListener = foundation.on(`collection:edit:${props.entity.toLowerCase()}`, async function (eventObj) {
    ...
    onDeleteDocEventListener = foundation.on(`collection:delete:${props.entity.toLowerCase()}`, async function (eventObj) {
      const { error /* , document, foundation,  data */ } = eventObj
      if (error) {
    ...
    return () => {
      ...
      foundation.stopListenTo(onAddDocEventListener)
      foundation.stopListenTo(onEditDocEventListener)
      foundation.stopListenTo(onDeleteDocEventListener)
    }
  }, []) // run one time only
